...
class MOEAD(AbstractGeneticAlgorithm):
    ...
    def initialize(self):
        ...
        #belegunduでmazda_mop.exeで全個体一括評価し
        #目的関数値を下のFOR分で与える
        ...
        # グラフを描画
        plt.scatter([s.objectives[0] for s in self.population if s.feasible],
               [s.objectives[1] for s in self.population if s.feasible])
        #plt.show()
        ...
        ax.set_title('result_plot')
        ax.set_xlabel = ('f1')
        ax.set_ylabel = ('f2')
        ax.scatter([s.objectives[0] for s in self.population if s.feasible],
               [s.objectives[1] for s in self.population if s.feasible])
        plt.savefig('data/mazda_pop/figure{}.png'.format(self.generation)
    ...
    def iterate(self):
        ...
        print("%d世代目更新終了"%(self.generation + 1))   
        ...
        ax = fig.add_subplot(1,1,1)
        ax.set_title('result_plot')
        ax.set_xlabel = ('f1:sum_weight')
        ax.set_ylabel = ('f2:commonparts')
        ax.scatter([s.objectives[0] for s in self.population if s.feasible],
               [s.objectives[1] for s in self.population if s.feasible])
        plt.savefig('data/mazda_pop/figure{}.png'.format(self.generation,"gen"))
